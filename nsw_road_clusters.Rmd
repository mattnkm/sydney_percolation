...
```{r}
library(librarian)
shelf(percopackage, sf, igraph, tidyverse, readxl)
getwd()
```

# data 
```{r}
au_net <- read_xlsx("./data/clean/au_edge_list.xlsx")
au_net <- au_net %>% select(edge_id, node_id, Length, x, y)
...
au_coords <- au_coords %>% select(node_id, x, y)
...
jumps_2plot <- c(100, 200, 300, 400, 500, 600, 800, 1000, 1500, 2500, 5000, 8000, 10000, 15000, 20000, 25000, 30000)
```
...
for (i_t in r0) {
    ...
    file_out <- paste(file_n_clust, i_t, ".txt", sep = "")
    write.table(sorted_table, file_out, row.names = FALSE, col.names = c("id_cluster",
        "n_points"))
...
```{r}
# shelf(reshape2, igraph, plyr, rgdal)
```
...
for (i_t in r0) {
    ...
    st_write(data_sf, paste("./data/percolation/results_coords/shp_top10/shp_top10_",
        i_t, ".shp", sep = ""))
